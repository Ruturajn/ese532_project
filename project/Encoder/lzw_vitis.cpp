...
void inline insert(unsigned long (*hash_table)[2], assoc_mem *mem, unsigned int key, unsigned int value,
                   bool *collision) {
...
static void lzw_compute(hls::stream<unsigned char> &in_stream, uint32_t start_idx, uint32_t end_idx,
                        hls::stream<uint32_t> &out_stream, uint32_t *code_length, uint8_t *failure,
...
static void load_data(unsigned char *chunk, uint32_t start_idx, uint32_t end_idx,
...
static void store_data(hls::stream<uint32_t> &out_stream, uint32_t temp_code_length, uint32_t *lzw_codes,
                       uint32_t *code_length, unsigned int temp_associative_mem, unsigned int *associative_mem,
                       uint8_t temp_failure, uint8_t *failure) {
...
#pragma HLS DATAFLOW
    ...
    store_data(out_stream, temp_code_length, lzw_codes, code_length, temp_associative_mem, associative_mem,
