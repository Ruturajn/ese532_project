...
typedef struct
{
    ...
    unsigned int value[ASSOCIATIVE_MEM_STORE];    // value store is 64 deep, because the lookup mems are 64 bits wide
...
void assoc_insert(assoc_mem* mem,  unsigned int key, unsigned int value, bool* collision)
{
    ...
    {
        mem->upper_key_mem[(key >> 18)%512] |= (1 << mem_fill);  // set the fill'th bit to 1, while preserving everything else
        mem->middle_key_mem[(key >> 9)%512] |= (1 << mem_fill);  // set the fill'th bit to 1, while preserving everything else
        mem->lower_key_mem[(key >> 0)%512] |= (1 << mem_fill);   // set the fill'th bit to 1, while preserving everything else
...
void assoc_lookup(assoc_mem* mem, unsigned int key, bool* hit, unsigned int* result)
{
    ...
    LOOP5: for(; address < ASSOCIATIVE_MEM_STORE; address++)
    {
